...
public class AdapterStoresListing extends BaseAdapter implements Filterable{
...
List<StoreList> filterList;
    ...
    public AdapterStoresListing(int source, Activity activity, List<StoreList> sideMenuItems, StoreList store) {
    ...
    private void initializeViews(final StoreList model, final ItemViewHolder holder, final int position) {
        ...
        if(pos!=-1) {
           // SharedPreference.getInstance(activity).setStore(C.SELECTED_STORE, filterList.get(pos));
            ((ActivityContainer)activity).enableButton();

        }
        else {
            ((ActivityContainer)activity).disableButton();
